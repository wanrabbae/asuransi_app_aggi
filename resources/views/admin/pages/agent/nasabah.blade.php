                    ...
                    <div class="widget-content widget-content-area br-8">
                        <form action="{{ url()->current() }}" method="get">
                            <div class="row py-4 px-4">
                                ...
                                <div class="col-xl-2 mb-4">
                                    <div class="form-group">
                                        <label for="totalnasabah"> Total Nasabah</label>
                                        <input readonly type="text" value="{{ $getJumlahAllNasabahByAgentReferalCode }}" id="totalnasabah" name="" class="form-control text-white bg-dark"
                                            placeholder="">
                                    </div>
                                </div>
                                <div class="col-xl-2 mb-4">
                                    <div class="form-group">
                                        <label for="totalincomeson">Total Online Premi</label>
                                        <input readonly type="text" value="{{ format_uang($sumTransactionByNasabahAgentOnline) }}" id="totalincomeson" name=""
                                            class="form-control text-white bg-dark" placeholder="">
                                    </div>
                                </div>
                                <div class="col-xl-2 mb-4">
                                    <div class="form-group">
                                        <label for="totalincomesoff">Total Offline Premi</label>
                                        <input readonly type="text" value="{{ format_uang($sumTransactionByNasabahAgentOffline) }}" id="totalincomesoff" name=""
                                            class="form-control text-white bg-dark" placeholder="">
                                    </div>
                                </div>
                            </div>
                        ...
                        @foreach ($transactionByNasabahAgent as $item)
                            <tr>
                                ...
                                <td>
                                    @if ($item->status == null)
                                        Offline
                                    @elseif($item->status_offline == null)
                                        online
                                    @endif
                                </td>
                                <td>{{ $item->product->name }}</td>
                                <td>{{ \Carbon\Carbon::parse($item->created_at)->format('d F Y') }}</td>
                                <td>{{ format_uang($item->nilai_premi) }}</td>
                                <td>
    ...
    @push('after-script')
        <script>
            ...
            downloadBtn.addEventListener('click', function() {
                ...
                var url = "/dashboard/userdata/nasabahExcel";
                var urlDownload = url + '?start_date=' + start_date + '&end_date=' + end_date + '&referalCodeUpline=' + '{{ $referalCodeUpline->referal_code }}';
